...
class ComputeInstancesGetSerialPortOutputRequest(messages.Message):
  ...
  instance = messages.StringField(1, required=True)
  port = messages.IntegerField(2, variant=messages.Variant.INT32, default=1)
  project = messages.StringField(3, required=True)
  zone = messages.StringField(4, required=True)
...
class Disk(messages.Message):
  ...
  Fields:
    ...
    description: An optional textual description of the resource; provided by
      the client when the resource is created.
    diskMasterKey: Specifies the master key to protect the disk. When
      attempting to use a disk that is protected by a master key, you must
      provide the correct key, otherwise, the request will fail. Master keys
      do not protect access to metadata about the resource.  If you are
    ...
    sourceImageMasterKey: Specifies the master key of the source image you are
      using. This key gives you access to the use the image and is required if
      the source image is master key protected.  The required key format is a
      random 256-bit string encoded in base64.
    ...
    sourceSnapshotMasterKey: Specifies the master key of the source snapshot
      you are using. This gives you access to the use the image and is
      required if the source snapshot is master key protected.  The required
...
class Image(messages.Message):
  ...
  Fields:
    ...
    id: [Output Only] Unique identifier for the resource; defined by the
      server.
    imageMasterKey: Specifies the master key to protect this image. When
      attempting to use an image that is protected by a master key, you must
      provide the correct key, otherwise, the request will fail. Master keys
      do not protect access to metadata about the resource.  If you are
      setting a master key, the required key format is a random 256-bit string
      encoded in base64.
    ...
    sourceDiskMasterKey: Specifies the master key of the source disk you are
      using. This field gives you access to the disk and is required if the
